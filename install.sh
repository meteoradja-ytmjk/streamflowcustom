...
GRAY='\033[38;5;245m'; BOLD='\033[1m';   DIM='\033[2m'; NC='\033[0m'

# ── Box geometry (ALL ASCII: + = - |) ────────────────────────────────────────
#   Total line width = 54 chars
#   "  +" (3) + 50 chars + "+" (1) = 54  → borders
#   "  |" (3) + " " (1) + %-48s  + " |" (2) = 54  → content
#
# Using ASCII +, =, -, | guarantees 1 char = 1 visual column on ALL terminals

B_TOP() { echo -e "  ${CYAN}${BOLD}+==================================================+${NC}"; }
B_MID() { echo -e "  ${CYAN}${BOLD}+--------------------------------------------------+${NC}"; }
B_BOT() { echo -e "  ${CYAN}${BOLD}+==================================================+${NC}"; }

# Left-aligned content line (txt must be ASCII-only, max 48 chars)
boxline() {
  local txt="$1" col="${2:-$WHITE}"
  printf "  ${CYAN}${BOLD}|${NC}${col} %-48s ${CYAN}${BOLD}|${NC}\n" "$txt"
}

# Center-aligned content line (txt must be ASCII-only)
boxline_c() {
  local txt="$1" col="${2:-$WHITE}"
  local len=${#txt}
  local lpad=$(( (48 - len) / 2 ))
  local rpad=$(( 48 - len - lpad ))
  printf "  ${CYAN}${BOLD}|${NC}${col}%*s%s%*s ${CYAN}${BOLD}|${NC}\n" \
    $(( lpad + 1 )) "" "$txt" "$rpad" ""
}
...
# Step function — box width 54 chars, all ASCII borders
# Top:     "  +--[ STEP 01/12 ]----------------------------------+"
#           3 + 2+1+1+5+5+1+1 + 34 + 1  =  54
# Content: "  |" (3) + " " (1) + %-49s (49) + "|" (1) = 54
# Bottom:  "  +" (3) + 50x"-" + "+" (1) = 54
step() {
  local NUM="$1" TEXT="$2"
  blank
  echo -e "  ${CYAN}${BOLD}+--[ ${ORANGE}${BOLD}STEP ${NUM}${CYAN}${BOLD} ]----------------------------------+${NC}"
  printf "  ${CYAN}${BOLD}|${NC} ${WHITE}${BOLD}%-49s${CYAN}${BOLD}|${NC}\n" " $TEXT"
  echo -e "  ${CYAN}${BOLD}+--------------------------------------------------+${NC}"
  blank
}
...
boxline_c "" ""
B_MID
OS_INFO=$(grep PRETTY_NAME /etc/os-release 2>/dev/null | cut -d= -f2 | tr -d '"' | cut -c1-40)
CPU_INFO="CPU: $(nproc 2>/dev/null || echo '?') core(s)  |  RAM: $(free -h 2>/dev/null | awk '/^Mem/{print $2}' || echo 'N/A')"
boxline "  ${OS_INFO}" "$GRAY"
boxline "  ${CPU_INFO}" "$GRAY"
B_BOT
...
command -v pm2 &>/dev/null || fail "PM2 tidak ditemukan. Coba: source ~/.bashrc"
...
B_TOP
boxline_c "" ""
boxline_c "INSTALASI BERHASIL SELESAI!" "${GREEN}${BOLD}"
boxline_c "" ""
B_MID
