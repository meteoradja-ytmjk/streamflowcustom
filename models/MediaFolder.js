...
class MediaFolder {
  static create({ name, user_id, type = 'media' }) {
    const id = uuidv4();
    const now = new Date().toISOString();

    return new Promise((resolve, reject) => {
      db.run(
        `INSERT INTO media_folders (id, name, user_id, type, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [id, name, user_id, type, now, now],
        function(err) {
          if (err) {
            console.error('Error creating media folder:', err.message);
            return reject(err);
          }
          resolve({ id, name, user_id, type, created_at: now, updated_at: now });
        }
      );
    });
  }
  ...
  static findByNameAndType(userId, name, type) {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT * FROM media_folders WHERE user_id = ? AND LOWER(name) = LOWER(?) AND type = ?',
        [userId, name, type || 'media'],
        (err, row) => {
          if (err) {
            console.error('Error finding media folder by name and type:', err.message);
            return reject(err);
          }
          resolve(row || null);
        }
